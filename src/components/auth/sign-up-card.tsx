...
const validationSchema = Yup.object({
  email: Yup.string()
    ...
    .test("domain-check", "Only company emails are accepted", (value) => {
  ...
  preferredUsername: Yup.string()
    ...
    .test(
      ...
      (value) => {
        if (!value) return false;
        // Check if the value contains at least two words (first name and surname)
        ...
        const { isSignUpComplete } = await signUp({
          ...
          options: {
            userAttributes: {
              ...
              preferred_username: values.preferredUsername, // Store full name
            },
            autoSignIn: true,
          },
        });
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-2.5">
          ...
          <div>
            <Input
              ...
              type="text"
              placeholder="Full Name (e.g., John Cross)"
              onChange={formik.handleChange}
              onBlur={formik.handleBlur}
              value={formik.values.preferredUsername}
              disabled={isSubmitting}
            />
            {formik.touched.preferredUsername &&
            formik.errors.preferredUsername ? (
              <div className="text-red-500 text-xs mt-1">
                {formik.errors.preferredUsername}
              </div>
            ) : null}
